...
1. 创建 proposed Note 前读取 `.agents/agent-note-workflow.json`。本仓库选择 `bilingual`，因此在 `.agents/notes/proposed/<class>/` 创建英文 `.md`、简体中文 `.zh.md` 和 `.i18n.yaml` 一致性记录三件套。
...
- 配置的 `proposedMode` 只控制 `proposed/` 下的文件。`proposed/` 之外的每份 Note 都是锁定双语三件套。
- 双语三件套中的英文和中文文件同等权威，标题层级、代码块和链接结构必须对应。
