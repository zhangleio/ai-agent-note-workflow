...
完整可运行的参考项目见 [examples](examples/README.zh.md)。其中包含待办事项 CLI 的多生命周期演进示例，以及文本统计 CLI 从需求澄清到实施验收的 feature 示例。
...
Bug-fix 必须先添加一个必然失败的回归测试，记录修复前的失败证据，再实施最小修复并让同一个测试转为通过。
...
`.i18n.yaml` 保存双方最后一次人工确认语义一致时的 Git blob hash。它使用严格 YAML 映射，每行包含一个不加引号的 Markdown 基名和一个 40 位小写 hash。允许注释与空行；拒绝 JSON 对象语法、带引号的键、重复键、未知键、嵌套值和行尾标点：
...
根指南、根 Agent 指令和示例索引也使用相邻的 `.i18n.yaml` 记录：
...
更新其中任一文档对的一种语言后，必须评审两份文档、刷新双方经过 LF 归一化的 Git blob hash，并运行：

```powershell
node scripts/verify-doc-i18n.mjs
```

该零依赖门禁检查三份记录、双向语言切换器、对应的标题与 fenced block 结构，以及 fenced 示例之外的真实相对链接。这些普通文档记录与 Agent Note 生命周期校验相互独立。
...
未批准前不修改产品代码。探索性原型如果已经存在，也不能因此跳过评审。
